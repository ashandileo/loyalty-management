import Divider from '../shared/Divider/Divider';

const VoucherDescription = () => {
  return (
    <>
      <p className="plus-jakarta-sans font-[400] text-[14px] leading-[23.8px] text-[#272A2D] text-justify mb-[32px]">
        Indulge in the ultimate coffee experience with our exclusive Starbucks
        Buy One Get One (BOGO) offer! Savor the rich flavors of your favorite
        handcrafted beverages while enjoying the delightful perk of getting a
        second one on the house. Embrace the joy of sharing and make every sip a
        moment to remember. Hurry, this limited-time offer is your perfect
        excuse to treat yourself and a friend to the comforting warmth and
        deliciousness of Starbucks. Terms and conditions apply; visit our stores
        or website for more details. Cheers to twice the enjoyment at Starbucks!
      </p>

      <Divider borderStyle="border-dashed" />
    </>
  );
};

export default VoucherDescription;
